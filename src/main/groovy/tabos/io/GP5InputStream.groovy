...
import tabos.Accentuation
import tabos.ArtificialHarmonic
import tabos.Barre
import tabos.Beat
import tabos.BeatStatus
import tabos.BeatStroke
import tabos.BeatStrokeDirection
import tabos.BendEffect
import tabos.BendPoint
import tabos.BendType
import tabos.Chord
import tabos.ChordAlteration
import tabos.ChordExtension
import tabos.ChordType
import tabos.Color
import tabos.Duration
import tabos.Fingering
import tabos.GraceEffect
import tabos.GraceEffectTransition
import tabos.GuitarString
import tabos.HeaderElement
import tabos.KeySignature
import tabos.LineBreak
import tabos.LyricLine
import tabos.Lyrics
import tabos.Marker
import tabos.Measure
import tabos.MeasureHeader
import tabos.MidiChannel
import tabos.MixTableChange
import tabos.MixTableItem
import tabos.NaturalHarmonic
import tabos.Note
import tabos.NoteEffect
import tabos.NoteType
import tabos.Octave
import tabos.PageSetup
import tabos.PinchHarmonic
import tabos.Pitch
import tabos.RSEEqualizer
import tabos.RSEInstrument
import tabos.RSEMasterEffect
...
import tabos.Track
import tabos.TrackRSE
import tabos.TrackSettings
import tabos.TremoloPickingEffect
import tabos.TrillEffect
import tabos.TripletFeel
import tabos.TupletBracket
import tabos.Velocities
import tabos.Voice
import tabos.VoiceDirection
import tabos.WahEffect
    ...
    Score readScore() {
        ...
        for (def header : score.measureHeaders) {
            for (def track : score.tracks) {
                ...
                for (int voiceIdx : (0..<Measure.MAX_VOICES)) {
                    ...
                    for (int beatIdx : (0..<readInt())) {
                        ...
                        beat.status = bool(beatFlags & 0x40) ? BeatStatus.from(read()) : beat.status
                        beat.duration.tap {
                            value = 1 << (read() + 2)
                            isDotted = bool(beatFlags & 0x01)
                            tuplet.tap {
                                if (bool(beatFlags & 0x20)) {
                                    enters = readInt()
                                    times = Integer.highestOneBit(enters)
                                }
                                it
                            }
                        }
                        beat.effect.chord = bool(beatFlags & 0x02) ? new Chord().tap {
                        ...
                        beat.text = bool(beatFlags & 0x04) ? readFixedLengthStringField() : beat.text
                        if (bool(beatFlags & 0x08)) {
                            short beatEffectFlags = readShort()
                            beat.effect.vibrato = bool(beatEffectFlags & 0x02)
                            beat.effect.fadeIn = bool(beatEffectFlags & 0x10)
                            if (bool(beatEffectFlags & 0x20)) {
                                beat.effect.slapEffect = SlapEffect.from(read())
                            }
                            if (bool(beatEffectFlags & 0x400)) {
                                beat.effect.tremoloBar = readInt().with { bendValue ->
                                    beat.effect.slapEffect ? null : new BendEffect(
                                        value: bendValue,
                                        type: BendType.DIP,
                                        points: [
                                            new BendPoint(0, 0),
                            ...
                            beat.effect.pickStroke = bool(beatEffectFlags & 0x200) ? BeatStrokeDirection.from(read()) : BeatStrokeDirection.NONE
                        }
                        ...
                        start += beat.status != BeatStatus.EMPTY ? beat.duration.toTime() : 0

                        // gp5 additions
                        // beat = getBeat(voice, start)
                        short gp5beatFlags = readShort()
                        beat.octave = switch (true) {
                        ...
                        beat.display.breakBeam = bool(gp5beatFlags & 0x0001)
                        beat.display.forceBeam = bool(gp5beatFlags & 0x0004)
                        beat.display.forceBracket = bool(gp5beatFlags & 0x2000)
                        beat.display.breakSecondaryTuplet = bool(gp5beatFlags & 0x1000)
